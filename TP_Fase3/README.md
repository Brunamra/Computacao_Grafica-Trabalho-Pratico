...
fase3/
├── engine/
│   ├── engine.cpp        # Renderização via VBO, animações, câmaras
│   ├── config/
│   │   ├── config.cpp     # Parsing XML, incluindo transformações com tempo
│   │   └── config.h
│   ├── tinyxml2/
│   └── utils/
│       ├── utils.cpp      # Álgebra vetorial e curvas de Catmull-Rom
│       └── utils.h
├── generator/
│   ├── generator.cpp
│   ├── primitives.cpp     # Inclui agora leitura de .patch e superfícies de Bézier
│   └── primitives.h
├── tests/                  # Ficheiros de configuração XML e de patch
│   ├── test_3_1.xml
│   ├── test_3_2.xml
│   ├── solar_system.xml
│   └── teapot.patch
├── scripts/
│   └── primitives_script.sh   # Suporta "phase_1", "phase_2", "phase_3" e "clean"
├── docs/
│   └── CG_TP_Fase3_Grupo12.pdf   # Relatório entregue nesta fase
├── CMakeLists.txt
└── .gitignore
```
...
- CMake ≥ 3.5
- OpenGL, GLUT e **GLEW** instalados no sistema (novo nesta fase, necessário para VBOs)
  - Linux (Debian/Ubuntu): `sudo apt install freeglut3-dev libglew-dev`
  - macOS: GLUT incluído via framework do sistema (GLEW não é necessário)
...
### Gerar primitivas (incluindo superfícies de Bézier)

```bash
./generator patch <ficheiro.patch> <tesselação> <filename>
```

Ou gerar o conjunto de primitivas usado nesta fase:

```bash
../scripts/primitives_script.sh phase_3
```

Para limpar os ficheiros `.3d` gerados:

```bash
../scripts/primitives_script.sh clean
```
...
O título da janela mostra em tempo real os FPS e o tempo decorrido da simulação.

**Controlos:** os mesmos da [Fase 2](../TP_Fase2/README.md) (câmara Explorer/FPS, modos de desenho, mudança de cor).

O relatório completo desta fase, com a explicação das superfícies de Bézier, dos VBOs e das transformações animadas, está disponível em [`docs/CG_TP_Fase3_Grupo12.pdf`](docs/CG_TP_Fase3_Grupo12.pdf).
